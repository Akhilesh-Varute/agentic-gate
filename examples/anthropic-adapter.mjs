...
import { handleResponse } from "../dist/adapters/anthropic.js"; // "agentic-gate/adapters/anthropic" once installed via npm
import { z } from "zod";

// Same scenario as examples/anthropic.mjs, but using the anthropic adapter to
// collapse the manual tool_use-parsing + message-rebuilding loop.
...
async function runEngine() {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    const response = await anthropic.messages.create({ model: MODEL_ID, max_tokens: 1024, messages, tools });
